...
if __name__ == '__main__':
    ...
    for dataset in different_sets:
        ...
        if dataset[0] == 'T':
            save_dir = os.path.join(os.getcwd(), 'saved_1d_cnn_models')
            model_name = 'keras_cnn_trained_model_epochs' + 'SC1' + '40' + '.h5'
            MODEL_PATH = save_dir+'/'+model_name
        else:
            save_dir = os.path.join(os.getcwd(), 'saved_1d_cnn_models')
            model_name = 'keras_cnn_trained_model_epochs' + str(dataset) + str(epochs) + '.h5'
            MODEL_PATH =save_dir+'/'+model_name
        # 对数据进行归一化处理
...
# model.add(Dense(num_classes, activation='softmax'))
        ...
        evaluate_results(test_y, test_y_predict)
