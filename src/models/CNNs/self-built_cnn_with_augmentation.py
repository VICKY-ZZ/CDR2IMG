...
import os
from src.models.utils import load_data, plot_confusion_matrix, evaluate_results, display_errors,load_data_with_diffent_ratio
...
for test_size in [(0.2, 0.25), (0.2, 0.5), (0.6, 0.5), (0.8, 0.5)]:
    test_s = test_size[0]
    val_s = (1-test_size[0])*test_size[1]
    train_s = (1-test_size[0])*(1-test_size[1])
    print('++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    print('Testing test:{} || val:{} || train:{}'.format(test_s, val_s, train_s))
    BATCH_SIZE = 8
    EPOCHS = 40
    KERNEL_SIZE = (6, 6)
    MODEL_PATH = "../trained_models/cnn_model_train={}_EPOCHS={}_BATCH_SIZE={}_KERNEL={}.h5".format( train_s, EPOCHS, BATCH_SIZE,KERNEL_SIZE)
    # MODEL_PATH = "../trained_models/cnn_trained_model.h5"
    # 1. Data preparation
    # Load data
    X_train,X_test,Y_train,Y_test =load_data_with_diffent_ratio('../../../data/processed_data/hour_trail/all.npz', test_size)
    #Split training and validation set
    X_train, X_val, Y_train, Y_val = train_test_split(X_train, Y_train, test_size = test_size[1], random_state=random_seed)
    print('train_y:{}:{}:{}'.format(np.sum(Y_train == 1), np.sum(Y_train == 0), len(Y_train)))
    print('val_y:{}:{}:{}'.format(np.sum(Y_val == 1), np.sum(Y_val == 0), len(Y_val)))
    print('test_y:{}:{}:{}'.format(np.sum(Y_test == 1), np.sum(Y_test == 0), len(Y_test)))
    # 2. CNN
    # set the model
